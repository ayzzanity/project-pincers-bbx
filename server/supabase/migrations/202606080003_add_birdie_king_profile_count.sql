...
tournament_swiss_bottoms as (
  select
    tournament_id,
    min(swiss_wins) as min_swiss_wins
  from verified_records
...
birdie_awards as (
  select
    v.user_id,
    count(*)::integer as birdie_king_count
  from verified_records v
  join tournament_swiss_bottoms b
    on b.tournament_id = v.tournament_id
   and b.min_swiss_wins = v.swiss_wins
  group by v.user_id
)
select
  p.id as user_id,
  ...
  coalesce(max(ba.birdie_king_count), 0)::integer as birdie_king_count
from public.profiles p
left join verified_records v on v.user_id = p.id
left join monthly_awards ma on ma.user_id = p.id
left join birdie_awards ba on ba.user_id = p.id
